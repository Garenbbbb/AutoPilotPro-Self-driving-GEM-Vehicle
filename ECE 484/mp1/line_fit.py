...
def line_fit(binary_warped):
	...
	for window in range(nwindows):
		...
		left_window_lower_left = (leftx_current - margin, y_low)
		left_window_upper_right = (leftx_current + margin, y_top)

		right_window_lower_left = (rightx_current - margin, y_low)
		right_window_upper_right = (rightx_current + margin, y_top)


		####
		# Draw the windows on the visualization image using cv2.rectangle()
		##TO DO
		cv2.rectangle(binary_warped, left_window_lower_left, left_window_upper_right, color=(255, 0, 0))
		cv2.rectangle(binary_warped, right_window_lower_left, right_window_upper_right, color=(255, 0, 0))
		...
		left_idxs = ((nonzeroy >= y_low) & (nonzeroy < y_top) & 
                          (nonzerox >= leftx_current - margin) & (
                           nonzerox < leftx_current + margin)).nonzero()[0]
		right_idxs = ((nonzeroy >= y_low) & (nonzeroy < y_top) & 
		...
		##TO DO
		left_lane_inds.append(left_idxs)
		right_lane_inds.append(right_idxs)

		####
		# If you found > minpix pixels, recenter next window on their mean position
		##TO DO
		if len(left_lane_inds) > minpix:
			leftx_current = int(np.mean(nonzerox[left_idxs]))
		
		if len(right_lane_inds) > minpix:
			rightx_current = int(np.mean(nonzerox[right_idxs]))
	...
	####
		...
		print("Unable to detect lanes", err)
		return None

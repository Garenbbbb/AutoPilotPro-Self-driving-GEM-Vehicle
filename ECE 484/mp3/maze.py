...
class Maze(object):
    ...
    def sensor_model(self, coordinates, sensor_limit, orientation = 0):
        ...
        pos_y = y
        d7 = 0
        dx = np.cos(orientation-5*np.pi/4) * 1 - np.sin(orientation+np.pi/2) * 0
        dy = np.sin(orientation-5*np.pi/4) * 1 + np.cos(orientation+np.pi/2) * 0
        ...
        pos_y = y
        d8 = 0
        dx = np.cos(orientation-7*np.pi/4) * 1 - np.sin(orientation+np.pi/2) * 0
        dy = np.sin(orientation-7*np.pi/4) * 1 + np.cos(orientation+np.pi/2) * 0
        ...
        # Return readings from sensor model in front, right, rear, left direction, front right, rear right, rear left, front left
